...
# passenger processes total private dirty.
#
...
get_vals() {
   passenger_memory_stats_passenger_processes_total_private_dirty=`sudo passenger-memory-stats | sed -n '/^-* Passenger processes -*$/,/^$/p' | grep "Total private dirty" | sed 's/.*: //; s/ //;'`
}

do_output() {
    output="$passenger_memory_stats_passenger_processes_total_private_dirty passenger memory stats passenger processes total private dirty"
}
